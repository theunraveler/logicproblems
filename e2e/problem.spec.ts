...
function getProofTable(page: Page) {
  return page.getByTestId('proof-table');
}

async function enterAndEnsureLine(
  proofTable: Locator,
  formula: string,
  rule: string,
  ...
  await test.step(`Fill in proof line ${expectedIndex}`, async () => {
    ...
    await expect(cols.nth(startingColumn + 2)).toHaveText(justifications.map((i) => i + 1).join(', '));
    await expect(cols.nth(startingColumn + 3)).toHaveText(rule);
  })
}

function ensureDialog(accept: boolean = true) {
